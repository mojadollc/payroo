...
import { getStoreId, getMainStoreId, switchBranch } from "@/lib/store-id"
import {
  listBranches,
  createBranch,
  deactivateBranch,
} from "@/lib/firebase/branch-services"
import type { StoreBranch } from "@/lib/firebase/types"
import { useAuth } from "@/hooks/use-auth"

export function BranchManager() {
  const { user } = useAuth()
  const { toast } = useToast()
  const [branches, setBranches] = useState<(StoreBranch & { isMain?: boolean })[]>([])
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!isOwner) {
      toast({ title: "Only the owner can add branches", variant: "destructive" })
      return
    }
    setSaving(true)
    try {
      await createBranch({
        ...
        branchExternalId: form.branchExternalId,
        address: form.address,
        phone: form.phone,
        ownerPin: form.ownerPin || undefined,
      })
      toast({
        title: "Branch created",
        description: `${form.branchName} (ID: ${form.branchExternalId}) is ready. Switch to it from the header.`,
      })
      setShowAdd(false)
  ...
  const handleDeactivate = async (b: StoreBranch & { isMain?: boolean }) => {
    if (b.isMain || !b.id || b.id === "main") return
    if (!confirm(`Deactivate branch "${b.branchName}"? Staff will no longer see it in the switcher.`)) return
    try {
      await deactivateBranch(b.id)
      toast({ title: "Branch deactivated" })
      if (activeId === b.branchExternalId) {
        switchBranch(getMainStoreId())
        return
      }
      await reload()
    } catch {
                ...
                <div className="flex flex-col gap-1 shrink-0">
                  {!isActive && (
                    <Button
                      ...
                      onClick={() => switchBranch(b.branchExternalId, b.branchName)}
                    >
                      Switch
                    </Button>
                  )}
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Creates a separate Store ID with its own products, sales, and stock. You can switch
              between branches without logging out.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreate} className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2">
                <Input
                  id="branchExternalId"
                  placeholder="4–8 digits/chars"
                  value={form.branchExternalId}
                  onChange={e =>
                    setForm({ ...form, branchExternalId: e.target.value.replace(/\s/g, "") })
                  }
                  required
                  minLength={4}
                  maxLength={8}
              ...
              <p className="text-[11px] text-muted-foreground">
                Staff can also log in directly with this ID + PIN.
              </p>
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                type="password"
                inputMode="numeric"
                placeholder="PIN for branch login"
                value={form.ownerPin}
