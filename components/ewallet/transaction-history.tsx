...
export function TransactionHistory({ transactions, cashinTransactions, isLoading, onRefresh }: TransactionHistoryProps) {
  ...
  const rows: Row[] = [
    ...transactions.map(t => ({ kind: "manual" as const, data: t, date: t.createdAt.toDate() })),
    ...cashinTransactions.map(t => ({ kind: "kiosk" as const, data: t, date: t.createdAt?.toDate?.() ?? new Date(0) })),
  ...
  return (
    <div className="space-y-3 max-h-[600px] overflow-y-auto">
      {rows.map((row) => {
            ...
            <div key={t.id} className="border rounded-lg p-4 hover:bg-muted/50 transition-colors">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className={`h-10 w-10 rounded-full flex items-center justify-center ${t.type === "cashin" ? "bg-secondary/20" : t.type === "load" ? "bg-purple-100" : "bg-primary/20"}`}>
                    {t.type === "cashin"
                ...
                <div className="flex items-start gap-2">
                  <div className="text-right">
                    ...
                    <div className="text-xs text-secondary font-semibold">+₱{Math.abs(t.profit).toFixed(2)} commission</div>
                  </div>
                  <Button
                    variant="ghost"
                    size="icon"
              ...
              {(t.customerName || t.customerNumber) && (
                <div className="mb-2 text-sm">
                  {t.customerName && <div className="text-muted-foreground"><span className="font-medium">Customer:</span> {t.customerName}</div>}
                  {t.customerNumber && <div className="text-muted-foreground"><span className="font-medium">Number:</span> {t.customerNumber}</div>}
                </div>
              )}
        ...
        // Kiosk transaction — no delete (managed by Xendit)
        const k = row.data
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <Badge variant="outline" className="uppercase">{k.channel}</Badge>
                  </div>
                  <p className="text-xs text-muted-foreground">{fmtDate(k.createdAt)}</p>
                </div>
              </div>
              <div className="text-right">
                <div className="font-bold text-lg">₱{(k.amountInserted ?? k.amount ?? 0).toLocaleString()}</div>
                <div className="text-xs text-secondary font-semibold">+₱{sellerEarning} your commission</div>
              </div>
            </div>
            {(k.accountName || k.accountNumber) && (
              <div className="mb-2 text-sm">
                {k.accountName && <div className="text-muted-foreground"><span className="font-medium">Customer:</span> {k.accountName}</div>}
                {k.accountNumber && <div className="text-muted-foreground"><span className="font-medium">Account:</span> {k.accountNumber}</div>}
              </div>
            )}
            <div className="flex items-center justify-between text-xs">
